...
try:
	...
	print('Make sure you have downloaded all required offices')
total=1
def Rep_Insta(total):
	user=input("[ ? ] Username > ")
	pess=input("[ ? ] Password > ")
	...
	while True:
		try:
			...
			if req.json()['status']=="ok":
				print(f"\r[{Fore.MAGENTA}${Fore.RESET}] {Fore.CYAN}Total number of reports{Fore.RESET} [{Fore.GREEN}{total}{Fore.RESET}]",end='')
				total+=1
			else:
				print(f'\n [!] Error Report')
...
{pyfiglet.figlet_format('IM RITH - SPAM')}
                     telegram : angkerith
                          By <\> @angkerith1
